...
public class XcomThreadPoolFactory {
    ...
    public static final ExecutorService IO_THEAD_POOL  = new XcomThreadPoolExecutor(IO_POOL_SIZE, IO_POOL_SIZE, 0L,
        TimeUnit.SECONDS, new LinkedBlockingQueue<>(QUEUE_SIZE), new ThreadFactoryBuilder().setNameFormat("io-pool-%d")
            .setUncaughtExceptionHandler(new XcomThreadExceptionHandler()).build());
    ...
    public static final ExecutorService CPU_THEAD_POOL = new XcomThreadPoolExecutor(CPU_POOL_SIZE, CPU_POOL_SIZE, 0L,
        TimeUnit.SECONDS, new LinkedBlockingQueue<>(QUEUE_SIZE), new ThreadFactoryBuilder().setNameFormat("cpu-pool-%d")
            .setUncaughtExceptionHandler(new XcomThreadExceptionHandler()).build());
    ...
    public static final ExecutorService JOB_THEAD_POOL = new XcomThreadPoolExecutor(IO_POOL_SIZE, IO_POOL_SIZE, 0L,
        TimeUnit.SECONDS, new LinkedBlockingQueue<>(QUEUE_SIZE * 2), new ThreadFactoryBuilder()
            .setNameFormat("job-pool-%d").setUncaughtExceptionHandler(new XcomThreadExceptionHandler()).build());
}
